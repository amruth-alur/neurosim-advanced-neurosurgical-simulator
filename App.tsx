...
import { 
  ...
  Scissors, Trash2, ZapIcon, Droplets, Pill, Scan, ShieldCheck, Trophy,
  ChevronDown
} from 'lucide-react';
...
// Fix for JSX intrinsic elements errors (like 'color') in React Three Fiber
declare global {
  namespace React {
    namespace JSX {
...
const App: React.FC = () => {
  ...
  const [activeTool, setActiveTool] = useState<string>('scalpel');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentCase.pathology.includes('Posterior')) icpRate = 0.07;
      if (currentCase.pathology.includes('Edema')) icpRate = 0.08;
  ...
  const handleSurgicalAction = (targetId: string, point?: any) => {
    ...
    if (activeTool === 'suture') {
        ...
        if (!allTreated) {
             ...
             logMsg = "The Dura must be opened and inspected before closure.";
             setConsultantAdvice("Reflect the Dura (Scalpel) to ensure no deep clots remain.");
        }
        else {
             if (isEdema) {
    ...
    // --- OTHER ACTIONS ---
    if (targetId === 'foreign-body' && activeTool === 'forceps') {
       ...
       markStepDone("Prep"); markStepDone("Remove"); markStepDone("Foreign");
    }
      ...
      if (!foreignBodyClear && currentCase.pathology.includes('Penetrating')) {
        ...
        markStepDone("Incise"); markStepDone("Open");
      }
    }
    else if (!surgicalLayers.dura && targetId !== 'brain-surface') {
        ...
        switch (activeTool) {
          ...
          case 'cautery':
            if (bleed.treatmentStage === 'suctioned') {
              ...
              markStepDone("Cauterize"); markStepDone("Seal");
            }
            break;
          case 'irrigation':
            if (bleed.treatmentStage === 'cauterized') {
              ...
              markStepDone("Dissect"); markStepDone("Wash");
            }
            break;
          default:
            logMsg = `The ${activeTool} is ineffective for this stage of hemostasis.`;
        }
        if (nextStage !== bleed.treatmentStage) updateBleed(targetId, { treatmentStage: nextStage });
      }
    } else if (!surgicalLayers.dura && targetId === 'brain-surface') {
       // Clicking the general surface with non-suture tools
       if (activeTool === 'irrigation') {
          logMsg = "Irrigating brain surface. Visibility improved.";
       } else {
          logMsg = "Select a specific bleed point to operate.";
       }
    } else {
  ...
  if (gameState === GameState.LOBBY) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-950 p-6 relative overflow-hidden">
        <div className="absolute inset-0 opacity-5 pointer-events-none"><Microscope size={800} className="text-white -translate-y-20 -translate-x-20" /></div>
        <div className="z-10 text-center max-w-4xl px-4">
           <div className="mb-10 flex justify-center scale-110">
              <div className="p-10 bg-blue-600/10 border-4 border-blue-600/30 rounded-[2.5rem] shadow-[0_0_150px_rgba(37,99,235,0.25)] animate-pulse"><Brain className="w-24 h-24 text-blue-500" /></div>
           </div>
           <h1 className="text-8xl font-black mb-6 tracking-tighter text-white">NEURO<span className="text-blue-500">CORE</span></h1>
           <p className="text-slate-400 text-2xl mb-8 font-light max-w-2xl mx-auto leading-relaxed">Advanced Neurosurgical Simulator. Trauma Protocol V2.1.</p>
           <div className="max-w-md mx-auto mb-10">
              ...
              <div className="relative">
                <select value={selectedScenario} onChange={(e) => setSelectedScenario(e.target.value)} className="w-full bg-slate-900 border border-slate-700 text-white rounded-xl px-4 py-3 appearance-none focus:outline-none focus:border-blue-500 transition-colors">
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none"><ChevronDown size={16} className="text-slate-400" /></div>
              </div>
           </div>
           <button onClick={startSurgery} disabled={loading} className="px-20 py-8 bg-blue-600 hover:bg-blue-500 rounded-3xl font-black text-3xl text-white transition-all hover:scale-105 active:scale-95 shadow-2xl disabled:opacity-50">{loading ? 'CALIBRATING...' : 'BEGIN SURGERY'}</button>
        </div>
      </div>
    );
  }
      ...
      <header className="px-10 py-6 border-b border-slate-800 bg-slate-950/90 backdrop-blur-3xl flex justify-between items-center z-50">
        <div className="flex items-center gap-12">
          <div className="flex items-center gap-5 border-r border-slate-800 pr-12">
             <div className="p-4 bg-blue-600/20 border-2 border-blue-600/30 rounded-2xl"><Microscope className="text-blue-500 w-7 h-7" /></div>
             <div>
                <h2 className="text-[11px] font-black tracking-[0.4em] uppercase text-blue-500 mb-1"> Trauma</h2>
                <div className="text-sm font-bold text-white tracking-widest uppercase flex items-center gap-2"><span className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />Live Procedure</div>
             </div>
          </div>
          <div className="hidden lg:flex gap-6">
             <div className="px-5 py-3 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col">
                ...
                <span className={`text-sm font-mono font-bold tracking-tighter ${vitals && vitals.intracranialPressure > 25 ? 'text-red-500' : 'text-emerald-500'}`}>{vitals ? (100 - (vitals.intracranialPressure - 10) * 2.5).toFixed(0) : '0'}%</span>
             </div>
             <div className="px-5 py-3 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col justify-center">
                <button onClick={() => setShowCT(true)} className="flex items-center gap-2 hover:text-blue-400 transition-colors"><Scan size={18} className="text-blue-500" /><span className="text-[10px] font-black uppercase tracking-wider">View CT Scan</span></button>
             </div>
             <div className="px-5 py-3 bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col">
                <button onClick={administerMeds} disabled={medsCooldown > 0} className={`flex items-center gap-2 transition-all ${medsCooldown > 0 ? 'opacity-30' : 'hover:text-blue-400'}`}><Pill size={16} className={medsCooldown > 0 ? '' : 'animate-bounce'} /><span className="text-[9px] font-black uppercase tracking-wider">Stabilize (Meds)</span>{medsCooldown > 0 && <span className="text-[10px] font-mono">[{medsCooldown}s]</span>}</button>
             </div>
          </div>
        </div>
        <button onClick={() => setGameState(GameState.LOBBY)} className="group p-4 bg-slate-900/50 border border-slate-800 rounded-2xl hover:bg-red-950/30 hover:border-red-500/50 transition-all"><RefreshCcw size={22} className="text-slate-400 group-hover:text-red-500 transition-colors" /></button>
      </header>
      ...
      <div className="flex-1 relative flex overflow-hidden">
        <aside className="w-[360px] border-r border-slate-800 bg-slate-950/60 p-6 flex flex-col gap-6 backdrop-blur-3xl">
           ...
           <div className="flex-1 flex flex-col bg-slate-900/30 border border-slate-800/50 rounded-[2.5rem] p-6 min-h-0">
              <div className="flex items-center gap-4 mb-4 border-b border-slate-800/40 pb-4"><Terminal size={18} className="text-blue-500" /><h3 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Surgical Log</h3></div>
              <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-hide">{logs.map((log, i) => (<div key={i} className="text-[11px] font-mono leading-relaxed border-l-2 border-blue-500/40 pl-3 py-1.5 bg-blue-500/5 rounded-r-lg"><span className="text-slate-300">{log}</span></div>))}</div>
           </div>
        </aside>

        <main className="flex-1 relative bg-black cursor-crosshair">
           <Canvas camera={{ position: [0, 0, 11], fov: 45 }} gl={{ antialias: true, localClippingEnabled: true }}>
              <color attach="background" args={['#1a1a1a']} /><Environment preset="city" /><Suspense fallback={null}>{currentCase && (<BrainModel bleeds={currentCase.bleeds} onInteractBleed={handleSurgicalAction} activeTool={activeTool} layers={surgicalLayers} pathology={currentCase.pathology} isForeignBodyRemoved={currentCase.isForeignBodyRemoved}/>)}</Suspense>
           </Canvas>
           <Loader />
           {showCT && currentCase && (<CTScanView bleeds={currentCase.bleeds} pathology={currentCase.pathology} report={currentCase.ctReport} onClose={() => setShowCT(false)} />)}
           <div className="absolute top-10 left-1/2 -translate-x-1/2 w-[600px] pointer-events-none">
              <div className="bg-slate-950/95 border border-blue-500/30 backdrop-blur-3xl p-6 rounded-[30px] shadow-2xl flex gap-6 pointer-events-auto">
                 <div className="p-4 bg-blue-600/10 border border-blue-600/20 rounded-2xl"><Zap className="text-blue-500 w-8 h-8" /></div>
                 <div><span className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-500 block mb-1">Sr. Consultant Instruction</span><p className="text-base text-blue-50 font-bold leading-tight">"{consultantAdvice}"</p></div>
              </div>
           </div>
           <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-4 p-4 bg-slate-950/90 backdrop-blur-3xl border border-slate-800 rounded-[40px] shadow-2xl">
              {SURGICAL_TOOLS.map(tool => (
                <button key={tool.id} onClick={() => setActiveTool(tool.id)} className={`group relative flex flex-col items-center justify-center w-20 h-20 xl:w-24 xl:h-24 rounded-[28px] transition-all duration-300 border-2 ${activeTool === tool.id ? 'bg-blue-600 border-blue-300 shadow-xl scale-110 -translate-y-4' : 'bg-slate-900 border-slate-800 hover:bg-slate-800'}`}>
                  ...
                  <span className={`text-[8px] font-black uppercase tracking-widest ${activeTool === tool.id ? 'text-white' : 'text-slate-500'}`}>{tool.id}</span>
                </button>
              ))}
           </div>
        </main>
        ...
        <aside className="w-[360px] border-l border-slate-800 bg-slate-950/60 p-6 flex flex-col gap-6 backdrop-blur-3xl">
           <div className="bg-slate-900/30 border border-slate-800/50 rounded-[2.5rem] p-6 flex-1 flex flex-col min-h-0">
              <div className="flex items-center gap-4 mb-6 border-b border-slate-800/40 pb-4"><FileText size={18} className="text-slate-500" /><h3 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Procedure Checklist</h3></div>
              {currentCase && (<div className="mb-4 p-3 bg-blue-950/30 border border-blue-900/50 rounded-xl"><h4 className="text-[9px] text-blue-400 font-bold uppercase mb-1">Primary Pathology</h4><p className="text-xs text-white font-mono">{currentCase.pathology}</p></div>)}
              <div className="space-y-6 overflow-y-auto scrollbar-hide pr-2">{currentCase?.surgicalSteps.map((step, i) => (<div key={i} className={`flex gap-4 items-start transition-all ${step.isCompleted ? 'opacity-30 grayscale' : 'opacity-100'}`}><div className={`mt-1 w-5 h-5 rounded-md border flex items-center justify-center shrink-0 ${step.isCompleted ? 'bg-emerald-500 border-emerald-500' : 'border-slate-700'}`}></div><div className="flex flex-col gap-1"><span className={`text-xs font-bold leading-snug ${step.isCompleted ? 'text-slate-500 line-through' : 'text-slate-100'}`}>{step.instruction}</span></div></div>))}</div>
           </div>
           <div className="bg-red-950/20 border border-red-900/40 p-5 rounded-2xl">
              <div className="flex items-center gap-3 mb-2"><ShieldAlert size={18} className="text-red-500" /><h3 className="text-[9px] font-black text-red-500 uppercase tracking-[0.2em]">Physiological Alert</h3></div>
              <p className="text-[11px] text-red-100/70 leading-relaxed italic">Cushing's triad detected. Keep ICP below 25mmHg to prevent brainstem herniation.</p>
           </div>
        </aside>
      </div>
      ...
      {gameState === GameState.DEBRIEF && (
        <div className="absolute inset-0 z-[200] bg-slate-950/98 backdrop-blur-3xl flex items-center justify-center p-12 text-center">
           <div className="max-w-2xl w-full">
              <AlertCircle size={80} className="text-red-600 mx-auto mb-8" /><h2 className="text-6xl font-black text-white mb-6 tracking-tighter uppercase">Case Terminated</h2>
              <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-8 mb-10 text-left">
                 ...
                 <div className="space-y-3">
                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-xs text-slate-400">Final ICP:</span><span className="text-sm font-mono text-red-500">{vitals?.intracranialPressure.toFixed(1)}</span></div>
                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-xs text-slate-400">Cause of Death:</span><span className="text-xs font-bold text-red-400 uppercase">Herniation / Cardiovascular Collapse</span></div>
                 </div>
                 <p className="mt-6 text-sm text-slate-400 italic">"You must work faster once the dura is opened. Every second of bleed-pool adds exponential ICP pressure. Use the stabilizer meds to buy time."</p>
              </div>
              <button onClick={() => setGameState(GameState.LOBBY)} className="px-16 py-6 bg-white text-slate-950 font-black text-xl rounded-2xl hover:bg-slate-200 transition-all shadow-2xl">RETURN TO HQ</button>
           </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-[200] bg-slate-950/98 backdrop-blur-3xl flex items-center justify-center p-12 text-center">
           <div className="max-w-3xl w-full">
              ...
              <div className="bg-emerald-950/30 border border-emerald-900/50 rounded-3xl p-8 mb-10 text-left relative overflow-hidden">
                 <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none"><Brain size={200} className="text-white" /></div>
                 ...
                 <div className="grid grid-cols-2 gap-8 mb-6 relative z-10">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-bold text-white uppercase flex items-center gap-2"><CheckCircle2 size={14} className="text-emerald-500" />Stable / Reactive Pupils</div>
                    </div>
                 </div>
                 <p className="mt-6 text-sm text-emerald-200/80 italic border-t border-emerald-900/30 pt-4">"Excellent work, Doctor. The hematoma was evacuated efficiently, and the dura was sealed perfectly. The patient is stable and responsive."</p>
              </div>
              <button onClick={() => setGameState(GameState.LOBBY)} className="px-16 py-6 bg-blue-600 text-white font-black text-xl rounded-2xl hover:bg-blue-500 transition-all shadow-2xl hover:scale-105">NEXT PATIENT</button>
           </div>
        </div>
      )}
    </div>
